...
: "${ARM64EC_EXCLUDE_SUBJECT_REGEX:=^(ntdll: (Store special environment variables with a UNIX_|Set the environment variables for Unix child processes from their UNIX_|Treat all the XDG_|Get rid of the wine_unix_to_nt_file_name syscall\\.|Get rid of the wine_nt_to_unix_file_name syscall\\.|Add NtConvertBetweenAuxiliaryCounterAndPerformanceCounter\\(\\) function\\.|Implement NtGetNextProcess\\.|Add a test for invalid syscall numbers\\.|Add NtAccessCheckByTypeAndAuditAlarm\\(\\) and NtCloseObjectAuditAlarm\\(\\) stubs\\.|Add stubs for some syscalls that need explicit ids\\.|Add semi-stub for NtQueueApcThreadEx2\\(\\)\\.|Implement NtAlertMultipleThreadByThreadId\\(\\)\\.|Add a stub for NtCreateSectionEx\\(\\)\\.|Add more LPC stubs\\.|Fix \\(Nt\\|Zw\\)WaitForMultipleObjects signature\\.|Add some new processor features definitions\\.|Implement WOW64 thread suspension helpers\\.)|tools: Add a platform-independent name for the ALL_SYSCALLS macros\\.|WIP: ntdll: ARM64EC suspend support)$}"
