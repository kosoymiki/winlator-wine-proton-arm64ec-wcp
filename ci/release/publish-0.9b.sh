...
WINLATOR_TAG="v0.9b"
WCP_TAG="wcp-stable"
WINLATOR_NOTES="${ROOT_DIR}/out/release-notes/winlator-v0.9b.md"
...
usage() {
  cat <<USAGE
Usage: bash ci/release/publish-0.9b.sh [--apply] [--repo owner/repo] [--winlator-tag TAG] [--wcp-tag TAG]

Publishes or updates the Winlator 0.9b release and versionless WCP stable release (dry-run by default).
USAGE
}
while [[ $# -gt 0 ]]; do
  case "$1" in
...
[[ -f "${WINLATOR_NOTES}" ]] || fail "Missing notes file: ${WINLATOR_NOTES} (run ci/release/prepare-0.9b-notes.sh)"
[[ -f "${WCP_NOTES}" ]] || fail "Missing notes file: ${WCP_NOTES} (run ci/release/prepare-0.9b-notes.sh)"
...
mkdir -p "${STAGE_DIR}/wcp-stable" "${STAGE_DIR}/winlator-v0.9b"
for apk in "${ROOT_DIR}/out/winlator"/*.apk; do
  ...
  cp -f "${apk}" "${STAGE_DIR}/winlator-v0.9b/Winlator by Aero.so-${apk_suffix}"
done
cp -f "${ROOT_DIR}/out/winlator/SHA256SUMS" "${STAGE_DIR}/winlator-v0.9b/SHA256SUMS.txt"
WINLATOR_ASSETS=("${STAGE_DIR}/winlator-v0.9b"/*.apk "${STAGE_DIR}/winlator-v0.9b/SHA256SUMS.txt")
...
if gh release view "${WINLATOR_TAG}" --repo "${REPO}" >/dev/null 2>&1; then
  gh release edit "${WINLATOR_TAG}" --repo "${REPO}" --title "Winlator by Aero.so 0.9b" --notes-file "${WINLATOR_NOTES}"
else
  gh release create "${WINLATOR_TAG}" --repo "${REPO}" --title "Winlator by Aero.so 0.9b" --notes-file "${WINLATOR_NOTES}"
fi
