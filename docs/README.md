...
- `ci/validation/collect-mainline-forensic-snapshot.sh` - one-shot collection of mainline health + active failures + Runtime Contract check logs/metadata (optional per-run triage + optional online-intake + optional release patch-base bundle capture).
- `ci/validation/prepare-release-patch-base.sh` - local patch-base release gate: runs audits, Runtime Contract, strict no-fetch intake/high-cycle, and optional Winlator patch-stack preflight / optional patch-base phase cycle into one bundle.
- `ci/validation/run-final-stage-gates.sh` - final pre-push gate runner chaining audits, GN manifest, Runtime Contract, strict intake/high-cycle, release-prep, and optional snapshot capture.
...
- `ci/reverse/run-high-priority-cycle.sh` - strict intake gate for high-signal repos/markers (`GN manifest + backlog strict + optional Runtime Contract smoke`).
