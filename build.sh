...
copy_usr_tree() {
  local src_root="$1"
  local dst_root="$2"
  if [[ -d "$src_root/usr/local" ]]; then
    cp -a "$src_root/usr/local/." "$dst_root/"
  elif [[ -d "$src_root/usr" ]]; then
    cp -a "$src_root/usr/." "$dst_root/"
  else
    die "Neither usr/local nor usr exists in staging root: $src_root"
  fi
}
...
if [[ ! -x "${LLVM_BIN}/clang" ]]; then
  ...
  [[ -n "$LLVM_SRC_DIR" ]] || {
    ...
    die "unable to locate llvm-mingw toolchain root after extraction"
  }

  cp -a "${LLVM_SRC_DIR}/." "$LLVM_DST/"
  [[ -x "${LLVM_DST}/bin/clang" ]] || die "llvm-mingw copied but clang is still missing in ${LLVM_DST}/bin"
fi
...
[[ -f "$CFG" ]] || die "customization.cfg not found at $CFG"
